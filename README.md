# PineTime LCD Driver for Mynewt

Simple no frills driver for Apache Mynewt. Design goals:

- small footprint
- nonblocking SPI to support
- almost no cost decompression
...
static uint8_t *next(uint8_t *buf, int *chunk, int remaining_len)
{
  ...
  static uint8_t *external_buf; /* this could be in flash */
  static int buf_idx=0; /* double buffering */
  int len;
  uint8_t *ret=NULL;
  ...
  if(NULL!=chunk)
  {
    ...
    memcpy(internal_buf[buf_idx], external_buf, len); /* if the external buffer is in flash
                                                       * it needs to be copied to RAM to be
                                                       * available for SPIM DMA */
    *chunk = len;
    external_buf += len;
  ...
  {
    /* initialize with a "chunk=NULL" call */
    external_buf = buf;
    buf_idx = 0;
  }
...
static void maintask(void *arg)
{
  ...
  /* display a full screen image stored in a flash array as
